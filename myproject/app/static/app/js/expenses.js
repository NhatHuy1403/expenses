let ExpenseController = (() => {
    ...
    return {
        inputEntry(userInput) {
            ...
            // Lưu ngày tháng năm chi tiêu
            let date = new Date(userInput['date']);
            let year = date.getFullYear();
            let month = date.getMonth();
            let day = date.getDate();
            // Lưu chi tiêu vào danh sách theo tháng/năm
            let monthKey = year + '-' + (month + 1);
            let item = {
                description: userInput['description'],
                value: userInput['value'],
                date: day + '-' + (month + 1) + '-' + year,
                type: userInput['expenseType']
            };
            if (!expenseList.hasOwnProperty(monthKey)) {
                expenseList[monthKey] = [];
            }
            expenseList[monthKey].push(item);
        },
